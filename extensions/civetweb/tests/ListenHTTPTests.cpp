 ...
 public:
  ListenHTTPTestsFixture()
   : tmp_dir(strdup("/tmp/gt.XXXXXX")) {
    ...
    port = std::uniform_int_distribution<uint16_t>(20000U,60000U)(rng);
    plan->setProperty(listen_http, "Listening Port", std::to_string(port));
  }
  ...
  void run_server() {
    ...
    std::string ssl_cert;
    bool isSecure = listen_http->getProperty("SSL Certificate", ssl_cert) && !ssl_cert.empty();
    std::string protocol = std::string("http") + (isSecure ? "s" : "");
...
TEST_CASE_METHOD(ListenHTTPTestsFixture, "HTTPS with client cert from bad CA", "[https]") {
  ...
  create_ssl_context_service("goodCA.crt", "badCA_goodClient.p12");
...
TEST_CASE_METHOD(ListenHTTPTestsFixture, "HTTPS with client cert with non-matching DN", "[https][DN]") {
  ...
  create_ssl_context_service("goodCA.crt", "goodCA_badClient.p12");
