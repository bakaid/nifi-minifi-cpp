...
namespace processors {
  ...
  void BaseOPCProcessor::onSchedule(const std::shared_ptr<core::ProcessContext> &context, const std::shared_ptr<core::ProcessSessionFactory> &factory) {
    ...
    if (!password_.empty() && (certpath_.empty() || keypath_.empty() || trustpath_.empty())) {
      logger_->log_error("Certificate path, Key path and Trusted server certificate path must be provided in case Password is provided!");
      return;
    }
    ...
    if (!certpath_.empty()) {
      ...
      if (trustBuffers_[0].empty()) {
        logger_->log_error("Failed to load trusted server cert from path: %s", trustpath_);
        return;
      }
    }
